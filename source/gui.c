...
static lv_obj_t *g_list_buttons[NUM_BUTTONS] = {0};
...
void list_button_event(lv_obj_t *obj, lv_event_t event) {
...
void arrow_button_event(lv_obj_t *obj, lv_event_t event) {
    if (event == LV_EVENT_KEY) {
        const u32 *key = lv_event_get_data();
...
void setup_buttons() {
    ...
    lv_obj_align(g_list_buttons[0], NULL, LV_ALIGN_IN_TOP_MID, 0, (LV_VER_RES_MAX - g_list_dscs[0].header.h * NUM_BUTTONS) / 2);
